...
void proc_init(pcb_t *proc) {
    ...
    memset((uint8_t*)(mem + frame*PAGE_SIZE), 0, PAGE_SIZE);
    fte_t *pt = frame_table + frame;
    /*
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    */
    vpn_t vaddr = vaddr_vpn(address);
    uint16_t offset = vaddr_offset(address);

    pte_t* pte = ((pte_t*) (mem + ((PTBR) * PAGE_SIZE))) + vaddr;

    if (pte->valid != 1) { //check for valid
        page_fault(address);
    }

    paddr_t p = (paddr_t) ((pte->pfn * PAGE_SIZE) + offset); //grab physical address

    fte_t *frame = (frame_table+pte->pfn); //Why does this only work with reference?
    frame->vpn = vaddr;
    frame->process = current_process;
    frame->referenced = 1;
...
void proc_cleanup(pcb_t *proc) {
    ...
    pfn_t pt = proc->saved_ptbr;
    /* Iterate the page table and clean up each valid page */
    for (int i = 0; i < NUM_PAGES;i++){
        pte_t *j = ((pte_t*)(mem+(pt*PAGE_SIZE))) + i;
        if (j -> valid == 1) {
            (*(frame_table+j -> pfn)).mapped = 0;
            j -> valid = 0;
            j -> dirty = 0;
            frame_table[j->pfn].mapped = 0;
        }
