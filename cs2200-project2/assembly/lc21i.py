...
"""lc21i.py: A definition of the LC-21i architecture."""
__author__ = "Christopher Tam/Adithya Nott/Kavin Krishnan/Mitchell Alvarado"\
        + "/Gavin Shek/Jacob Meyers/Harry Wang"
...
class bge(Instruction):
    __RE_BGE = re.compile(r'^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*$')
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        match = cls.__RE_BGE.match(operands)
        if match is None:
            raise RuntimeError("Operands '{}' are in an incorrect format.".format(operands.strip()))
...
class ble(Instruction):
    __RE_BLE = re.compile(r'^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*$')
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        match = cls.__RE_BLE.match(operands)
        if match is None:
            raise RuntimeError("Operands '{}' are in an incorrect format.".format(operands.strip()))
